...
import { BAKED_ENTRY } from '../lib/baked-entry.js';
import { summarizeReviews } from '../lib/reviews.js';
import { extractEntry } from '../lib/entry-extract.js';
...
const entry = { ...BAKED_ENTRY };
let ok = 0, none = 0, i = 0, entryN = 0;
for (const id of need.slice(0, LIMIT)) {
  ...
  try {
    ...
    const entTag = ent ? ` · entry ${ent.eur === 0 ? 'Free' : '€' + ent.eur}${ent.basedOn ? '(' + ent.basedOn + ')' : ' default'}` : '';
    console.log(`  [${i}] ${review ? '✓' : '·'} ${v.name} (${v.city}) — ${review?.basedOn || 0} reviews, ${nPros} pts${review?.summary ? ', desc' : ''}${entTag}`);
    if (DRY) { await sleep(90); continue; }
    map[id] = { ...b, rating, ratings, review: review || b.review || null };
    if (ent) { entry[id] = ent.eur; entryN++; }
...
if (!DRY && entryN) {
  fs.writeFileSync(path.join(__dirname, '..', 'lib', 'baked-entry.js'),
    '// AUTO-GENERATED by scripts/bake-reviews.js — entry/cover price (EUR base)\n// derived from what real Google reviewers say about each venue\'s door charge.\n// 0 = free entry. Absent = no review signal (keep the seeded baseline).\n'
    + 'export const BAKED_ENTRY = ' + JSON.stringify(entry) + ';\n');
  console.log(`Set entry price for ${entryN} venues from reviews. Wrote baked-entry.js.`);
}
